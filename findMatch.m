function bestMatchID = findMatch(clip, fs,hash_table,numSongs)
%  findMatch() takes a sound clip and the sampling frequency and
%  attempts to find the best match within the loaded song database
% returns mxn of hash_table 
...
% Get the id of the best match (one with the most occurences of a certain offset).
[m, i] = max(modes(:, 2));
bestMatchID = i;
end